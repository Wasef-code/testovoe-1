...
from domain.deps.deps_interfaces import get_mail
...
class AuthService:
    ...
    def confirm(self, identifier: str, code: int):
        ...
        self.repo_object.update(self.repo_object, {"activated": True})
    ...
    def login(self, login_user: LoginUserModel):
        ...
        user = get_user_repo().get_user_by_phone(login_user.phone_number) if \
            is_phone else get_user_repo().get_user_by_email(login_user.email)
        if user is None:
    ...
    def refresh_token(token: str):
        token = refresh_jwt(token, get_user_repo())
        if token == "":
