...
from models.models import AccountDB
...
class AccountRepository:
    ...
    def create_account(self, user_id: str, currency_tag: str) -> int:
        i = generate_id()
    ...
    def get_account(self, account_id: str) -> AccountDB:
        try:
            return self.__accounts[account_id]
        except KeyError:
            return AccountDB()

    def get_user_accounts(self, user_id: str) -> list[AccountDB]:
        l: list[AccountDB] = []
    ...
    def transfer(self, first_id: str, second_id: str, amount: float,
                    cur_repo: CurrencyRepository, trans_repo: TransactionRepository) -> bool:
        charge = False
        try:
            if first_id == "-1":
                second = self.__accounts[second_id]
                second.amount += amount
                trans_repo.create_transaction(amount, datetime.now(), second.currency_tag, account_to_id=second_id)
                return True
            elif second_id == "-1":
                ...
                if first.amount <= amount:
                    return False
                first.amount -= amount
                trans_repo.create_transaction(amount, datetime.now(), first.currency_tag, account_from_id=first_id)
                return True
            ...
            trans_repo.create_transaction(amount, datetime.now(), first.currency_tag, first_id, second_id)
            return True
        except KeyError:
            print("KEY ERROR")
