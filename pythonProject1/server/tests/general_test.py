...
def test_account_creation():
    ...
    response = requests.post(f"http://localhost:8000/account/deposit/\
        {accs['USD']}", json={
        "token": token,
        "amount": 1500
    })
    assert response.json()["Message"] == "Success"
    # THIS TEST IS BUGGED, SOMEWHY FASTAPI THROWS "BODY - EXPECTED STR" error
    ...
    print(response.json())
    assert response.json()["Message"] == "Success"'''
    response = requests.post(f"http://localhost:8000/account/{accs['EUR']}",
                             json=token)
    print(response.json())
